...
export const MessageList = ({ messages, isStreaming, }) => {
    ...
    /** Format nanoseconds to seconds or minutes (e.g. "0.13s", "1.2s", "2.5 min") */
    const formatDurationSeconds = (ns) => {
        if (ns == null || ns === 0)
            return null;
        const sec = ns / 1e9;
        if (sec >= 60) {
    ...
    const renderResponseMetrics = (metrics) => {
        if (!metrics)
            return null;
        const parts = [];
        if (metrics.totalDurationNs != null) {
            const s = formatDurationSeconds(metrics.totalDurationNs);
            if (s)
                parts.push(`Total ${s}`);
        }
        if (metrics.promptEvalCount != null && metrics.promptEvalCount > 0) {
            const durationNs = metrics.promptEvalDurationNs;
            const timeStr = durationNs != null ? formatDurationSeconds(durationNs) : null;
            const tokensPerSec = durationNs != null && durationNs > 0
                ? (metrics.promptEvalCount / (durationNs / 1e9)).toFixed(1)
                : null;
            let p = `Prompt ${metrics.promptEvalCount} tokens`;
            if (timeStr)
                p += ` (${timeStr})`;
            if (tokensPerSec)
                p += ` @ ${tokensPerSec} tok/s`;
            parts.push(p);
        }
        if (metrics.evalCount != null && metrics.evalCount > 0) {
            const durationNs = metrics.evalDurationNs;
            const timeStr = durationNs != null ? formatDurationSeconds(durationNs) : null;
            const tokensPerSec = durationNs != null && durationNs > 0
                ? (metrics.evalCount / (durationNs / 1e9)).toFixed(1)
                : null;
            let p = `Output ${metrics.evalCount} tokens`;
            if (timeStr)
                p += ` (${timeStr})`;
            if (tokensPerSec)
                p += ` @ ${tokensPerSec} tok/s`;
        ...
        if (metrics.loadDurationNs != null && metrics.loadDurationNs > 0) {
            const s = formatDurationSeconds(metrics.loadDurationNs);
            if (s)
                parts.push(`Load ${s}`);
        }
        if (metrics.doneReason && metrics.doneReason !== 'stop')
            parts.push(`Done: ${metrics.doneReason}`);
        if (parts.length === 0)
            return null;
        return _jsx("div", { className: "mt-2 pt-2 border-t border-gray-100 text-[11px] text-gray-400 font-normal", children: parts.join(' · ') });
    };
    const renderMessage = (message) => {
        const isUser = message.role === 'user';
        const isSystem = message.role === 'system';
        const metrics = !isUser && message.metadata?.responseMetrics ? renderResponseMetrics(message.metadata.responseMetrics) : null;
