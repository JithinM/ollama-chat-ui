...
/** Ollama /api/chat request (https://docs.ollama.com/api/chat) */
...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (
    ...
    const attachmentBlock =
      files?.length && files.some((f) => f.content)
        ? '\n\n' +
          files
            .filter((f) => f.content)
            .map((f) => `[File: ${f.name}]\n${f.content}`)
    ...
    try {
      ...
      await apiClient.postStream('/api/chat', body, (chunk) => {
        ...
        if (chunk.done) {
          ...
          lastTotalDuration = (chunk.total_duration as number) ?? 0;
        }
      });
